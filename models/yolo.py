...
class DWDetect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        self.m = nn.ModuleList(DWConv(x, self.no * self.na, 1) for x in ch)  # output conv
...
class DWASFF_Detect(nn.Module):  # add ASFFV5 layer and Rfb
    stride = None  # strides computed during build
    onnx_dynamic = False  # ONNX export parameter
    export = False  # export mode
    ...
    def __init__(self, nc=80, anchors=(), ch=(), multiplier=0.5, rfb=False, inplace=True):  # detection layer
        ...
        self.na = len(anchors[0]) // 2  # number of anchors
        self.grid = [torch.zeros(1)] * self.nl  # init grid
        self.l0_fusion = DWASFFV5(level=0, multiplier=multiplier, rfb=rfb)
        self.l1_fusion = DWASFFV5(level=1, multiplier=multiplier, rfb=rfb)
        self.l2_fusion = DWASFFV5(level=2, multiplier=multiplier, rfb=rfb)
        self.anchor_grid = [torch.zeros(1)] * self.nl  # init anchor grid
        self.register_buffer('anchors', torch.tensor(anchors).float().view(self.nl, -1, 2))  # shape(nl,na,2)
        self.m = nn.ModuleList(nn.Conv2d(x, self.no * self.na, 1) for x in ch)  # output conv
        self.inplace = inplace  # use in-place ops (e.g. slice assignment)
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect)or isinstance(m, ASFF_Detect)or isinstance(m,Decoupled_Detect) or isinstance(m, DWASFF_Detect):
            s = 256  # 2x min stride
    ...
    def _profile_one_layer(self, m, x, dt):
        c = isinstance(m, Detect) or isinstance(m, ASFF_Detect)  or isinstance(m, DWASFF_Detect)# is final layer, copy input as inplace fix
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP,CBAM,ResBlock_CBAM,
                 CoordAtt,CrossConv,C3,CTR3,conv_bn_relu_maxpool,Involution, C3SPP, ShuffleNetV2_InvertedResidual,C3Ghost, CARAFE,Conv3BN, InvertedResidual]:
            c1, c2 = ch[f], args[0]
            ...
            if isinstance(args[1], int):  # number of anchors
                args[1] = [list(range(args[1] * 2))] * len(f)
        elif m is DWASFF_Detect or (m is Decoupled_Detect):
            args.append([ch[x] for x in f])
            if isinstance(args[1], int):  # number of anchors
                args[1] = [list(range(args[1] * 2))] * len(f)
        elif m is Contract:
...
if __name__ == '__main__':
    #yolov5s_bifpn
    # yolov5s_bifpn
    #yolov5s_mymobilenetv3_DWasff
    # yolov5s_mymobilenetv3_DWasff_bifpn
    # yolov5s_gam   11468919
    # yolov5s  7634541
    # yolov5s_shufflenetv2

    parser = argparse.ArgumentParser()
    parser.add_argument('--cfg', type=str, default='/home/atr2/Code/yolov5_research-master/models/yolov5s_mymobilenetv3_DWasff.yaml', help='model.yaml')
