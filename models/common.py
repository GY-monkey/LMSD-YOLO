...
#     def __init__(self):
#         super().__init__(DSConv,self).__init__()
#


class DSConv(nn.Module):
    # Standard convolution
    def __init__(self, c1, c2, k=1, s=1, p=None, groups=1, act=True,bias=False):  # ch_in, ch_out, kernel, stride, padding, groups
        super(DSConv, self).__init__()
        self.depthwiseconv = nn.Conv2d(c1, c1, k, s, autopad(k, p), groups=c1, bias=bias)
        self.pointwiseconv = nn.Conv2d(c1,c2,kernel_size=1)
        self.bn = nn.BatchNorm2d(c2)
        # self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.act = MetaAconC(c1 = c2) if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
    ...
    def __init__(self, inp, oup, hidden_dim, kernel_size, stride, use_se, use_hs):
        ...
        if inp == hidden_dim:
            self.conv = nn.Sequential(
                ...
                DWConv(hidden_dim, hidden_dim, kernel_size, stride, (kernel_size - 1) // 2),
                nn.BatchNorm2d(hidden_dim),
                h_swish() if use_hs else nn.ReLU(inplace=True),   ###激活函数的定义
                # Squeeze-and-Excite
                SELayer(hidden_dim) if use_se else nn.Sequential(),
                # pw-linear
                # nn.Conv2d(hidden_dim, oup, 1, 1, 0, bias=False),
                DWConv(hidden_dim, oup, 1, 1, 0),
                nn.BatchNorm2d(oup),
            )
        else:
            self.conv = nn.Sequential(
